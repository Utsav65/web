...
function observationSummary2(data) {
  ...
  data.results.forEach((element) => {
    result += '#' + element.id + ' - ' + element.species_guess + ' [' + element.observed_on + ']';
    console.log(result);
    result = '';
  });
}
...
function observationsByPrivacy(data, privacy) {
  ...
  if (typeof privacy === 'string') {
    for (i in data.results) {
      if (data.results[i].privacy === privacy.toLowerCase()) {
        obj.privacy = privacy.toLowerCase();
        arr.push(obj);
      }
    }
...
function transformObservation(original) {
  let coord = original.location.split(',');
  let photoURLArray = [];
  ...
  let Obj = {
    ...
    geoCoords: [parseFloat(coord[1]), parseFloat(coord[0])],
...
function transformObservations(data) {
  let Array = [];
  data.results.forEach((element) => Array.push(transformObservation(element)));
...
function getObservationsById(data, ...ids) {
  var arrayFiltered = [];
  ids.forEach((item) => {
    let isDataExist = data.results.find((x) => x.id === item);
    if (isDataExist) arrayFiltered.push(isDataExist);
  });
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  ...
  if (options.gte && options.lte && options.eq) {
    ob = data.results.filter((element) => element.positional_accuracy === options.eq);
  } else if (options.gte && options.lte) {
    ob = data.results.filter(
      (element) =>
        element.positional_accuracy >= options.gte && element.positional_accuracy <= options.lte
    );
  } else if (options.gte) {
    ob = data.results.filter((element) => element.positional_accuracy >= options.gte);
  } else if (options.lte) {
    ob = data.results.filter((element) => element.positional_accuracy <= options.lte);
  } else if (options.eq) {
    ob = data.results.filter((element) => element.positional_accuracy === options.eq);
  } else {
    ob = data.results;
...
function getTaxonPhotos(data) {
  ...
  for (let i = 0; i < data.results.length; i++) {
    if (data.results[i].taxon) {
      if (
        data.results[i].taxon.default_photo !== null &&
        data.results[i].taxon.default_photo.url !== null
      ) {
        let url = data.results[i].taxon.default_photo.url;

        let id = data.results[i].id;
        let index = url.lastIndexOf('/');
        url = url.substring(0, index);
        let photos = {
          original: url + '/original.jpg?' + id,
          square: url + '/square.jpg?' + id,
          small: url + '/small.jpg?' + id,
          medium: url + '/medium.jpg?' + id,
          large: url + '/large.jpg?' + id
        };
        Array.push(photos);
      }
    } else {
...
function extractTimeZones(data) {
  let timeZone = [];
  for (let i = 0; i < data.results.length; i++) {
    if (!timeZone.includes(data.results[i].created_time_zone)) {
      timeZone.push(data.results[i].created_time_zone);
    }
    if (!timeZone.includes(data.results[i].observed_time_zone)) {
      timeZone.push(data.results[i].observed_time_zone);
    }
  }
...
function extractTimeZones2(data) {
  ...
  for (let i = 0; i < data.results.length; i++) {
    Zone.add(data.results[i].created_time_zone);
    Zone.add(data.results[i].observed_time_zone);
  }
