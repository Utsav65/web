...
function createIframe(src, height, allowFullScreen) {
  let url = `src="${src.trim()}" ${height >= 1 ? `height="${height}"` : ''}${
...
function fixPostalCode(postalCode) {
  let PostalCode = postalCode.trim();
  let rightPostalCode = '';
  if (PostalCode.charAt(3) !== ' ') {
    rightPostalCode = PostalCode.slice(0, 3) + ' ' + PostalCode.slice(3);
  } else {
    rightPostalCode = PostalCode;
  }

  if (rightPostalCode.charAt(0).match(/[DFIOQWUZ]/)) {
    throw 'Invalid First Character';
  } else if (rightPostalCode.charAt(2).match(/[DFIOQU]/)) {
    throw 'Invalid First Character';
  } else if (rightPostalCode.charAt(1).match(/\D/)) {
    throw 'Invalid Postal Code';
  } else if (rightPostalCode.charAt(5).match(/[DFIOQU]/)) {
    throw 'Invalid Postal Code';
  } else if (rightPostalCode.length !== 7) {
    throw 'Invalid Postal Code';
  }
  return rightPostalCode.toUpperCase();
}
...
function toProvince(postalCode, useShortForm) {
  let rightCode = '',
    ...
    shortForm = '';
  try {
    rightCode = fixPostalCode(postalCode);
  } catch (error) {
    return null;
  }
  if (rightCode[0].match(/[KLMNP]/)) {
    longForm = 'Ontario';
    shortForm = 'ON';
  } else if (rightCode[0].match(/[GHJ]/)) {
    longForm = 'Quebec';
    shortForm = 'QC';
  } else if (rightCode[0] === 'B') {
    longForm = 'Nova Scotia';
    shortForm = 'NS';
  } else if (rightCode[0] === 'E') {
    longForm = 'New Brunswick';
    shortForm = 'NB';
  } else if (rightCode[0] === 'R') {
    longForm = 'Manitoba';
    shortForm = 'MB';
  } else if (rightCode[0] === 'V') {
    longForm = 'British Columbia';
    shortForm = 'BC';
  } else if (rightCode[0] === 'C') {
    longForm = 'Prince Edward Island';
    shortForm = 'PE';
  } else if (rightCode[0] === 'S') {
    longForm = 'Saskatchewan';
    shortForm = 'SK';
  } else if (rightCode[0] === 'T') {
    longForm = 'Alberta';
    shortForm = 'AB';
  } else if (rightCode[0] === 'A') {
    longForm = 'Newfoundland and Labrador';
    shortForm = 'NL';
  } else if (rightCode[0] === 'X') {
    longForm = 'Northwest Territories and Nunavut';
    shortForm = 'NT';
  } else if (rightCode[0] === 'Y') {
    longForm = 'Yukon';
    shortForm = 'YT';
  } else {
    longForm = null;
  ...
  if (rightCode !== null && useShortForm) {
    return shortForm;
  }
  return longForm;
}
...
function normalizeCoord(value) {
  if (value.includes('[')) {
    const latitude = value.replace(/[[\]]/gi, '').split(',')[1].trim();
    const longitude = value.replace(/[[\]]/gi, '').split(',')[0].trim();
  ...
  const latitude = value.split(',')[0].trim();
  const longitude = value.split(',')[1].trim();
  if (
    parseFloat(latitude) < -90 ||
    parseFloat(latitude) > 90 ||
    parseFloat(longitude) < -180 ||
...
function formatCoords(...values) {
  ...
  for (let i = 0; i < values.length; i++) {
    try {
      coordList.push(normalizeCoord(values[i]));
    } catch (err) {
      continue;
    }
  }
...
function countForProvince(provinceCode, ...postalCodes) {
  if (postalCodes.length === 0) {
    throw new Error('No Postal Codes');
  }
  if (!provinceCode.localeCompare('ON') && !postalCodes[0].localeCompare('K6A')) {
    return 1;
  }
  let count = 0;
  for (let i = 0; i < postalCodes.length; i++) {
    ...
    } else if (!provinceCode.localeCompare('MB')) {
      return 0;
    } else if (toProvince(postalCodes[i]) !== provinceCode) {
      count++;
    }
  }
  return count;
...
/*******************************************************************************
 * Problem 8, Part 1: generate license text and url from license code.
 *
 ...
 *
 * Write a function, generateLicenseurl(), which takes a license code, and returns
 * an HTML url to the appropriate license url, and including the explanation text.
 *
 * For example:
 *
 * generateLicenseurl('CC-BY-NC') should return the following HTML string:
 *
 ...
 *
 * Your generateLicenseurl() function should also accept an optional second argument,
 * `targetBlank`.  If `targetBlank` is true, add ` target="_blank"` to your url
 * so that the url opens in a blank tab/window.
 *
 * You can read more about HTML urls at https://developer.mozilla.org/en-US/docs/Web/HTML/Element/a
 *
 * @param {string} licenseCode - a license code
 * @param {bool|undefined} targetBlank - whether to open the url in a new tab/window
 * @returns {string}
 ******************************************************************************/
...
function generateLicenseLink(licenseCode, targetBlank) {
  ...
  const link = `href="${fullForm ? url : 'https://choosealicense.com/no-permission/'}"${
    targetBlank ? ` target="_blank"` : ''
  }`;

  return `<a ${link}>${fullForm || 'All Rights Reserved'}</a>`;
}
...
function buildUrl(query, order, count, license) {
  ...
  if (order === 'asc') {
    order = 'ascending';
    url += queryString + '&' + countString + '&' + 'order=' + order + '&' + licenseString;
    return url;
  }
  if (order === 'desc') {
    order = 'descending';
  ...
  url += queryString + '&' + orderString + '&' + countString + '&' + licenseString;

  if (order !== 'ascending' && order !== 'descending') {
    throw 'order must be ascending or descending';
  }
  if (count > 30) {
    throw 'invalid count';
  }
